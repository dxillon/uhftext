...
const Navbar = () => {
  ...
  const CircleArrowOutUpRight = ({
    ...
    <svg
      ...
      className={`lucide lucide-circle-arrow-out-up-right-icon ${className}`}
      {...props}
    >
  ...
  const Theater = (props: React.SVGProps<SVGSVGElement>) => (
    <svg
      ...
      stroke-linecap="round"
      stroke-linejoin="round"
      class="lucide lucide-theater-icon lucide-theater"
    >
  ...
  useEffect(() => {
    if (isOpen) {
      document.body.style.overflow = "hidden";
      document.documentElement.style.overflow = "hidden"; // For good measure
    } else {
      document.body.style.overflow = "auto";
    ...
    <nav className="fixed w-full z-50 mt-5">
      <div className="max-w-screen-4xl mx-auto px-8 sm:px-10 lg:px-18 relative">
        ...
        <div className="relative z-1 flex items-center justify-between h-12 md:h-20 px-0">
            ...
            >
              {updates.map((item, index) => (
                ...
                >
                  ...
                  >
                    <span className="text-white font-medium text-s whitespace-nowrap overflow-hidden text-ellipsis">
                      {item.text}
                    </span>
          ...
          {/* Desktop Nav */}
          <div className="hidden md:flex items-center space-x-8 -mr-6.5">
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  <div
                    className={link.label === "Hall" ? "relative top-0.5" : ""}
                  >
                    <Icon className="w-6 h-6" />
                  </div>
                  <span className="  text-[12px] mt-1 font-medium  tracking-wider  leading-relaxed   font-sans  text-white  drop-shadow-md  ">
                    {link.label}
                  </span>
                </Link>
              );
            ...
            <Link
              ...
              style={{
                ...
                alignItems: "center",
                gap: "5px",
                padding: "11px 12px 10px 6px", // Matches py-2.5 and px-3
                borderRadius: "9999px",
                background: "rgba(255, 255, 255, 0.08)",
                backdropFilter: "blur(10px)",
                WebkitBackdropFilter: "blur(10px)",
                color: "white",
                fontSize: "20px", // Matches text-[22px]
                fontWeight: "400",
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.boxShadow =
                  "0 6px 16px rgba(250, 126, 30, 0.4)";
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.boxShadow =
            ...
            >
              ...
              <span
                style={{
                  ...
                  width: "24px", // Match icon size from <LibraryBig />
                  height: "24px",
                  background:
              ...
              />
              <span style={{ lineHeight: "1.3" }}>GET CASTED</span>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.3 }}
            className="fixed inset-0 z-9 bg-black/30 backdrop-blur-md"
          >
            <motion.div
              initial={{ y: -50 }}
              animate={{ y: 0 }}
              ...
              className="relative pb-0.5 mt-16 ml-20 mr-auto p-4 rounded-xl bg-black-900/90 border border-gray-800 backdrop-blur-md w-[75%] max-w-xs" // Changed here
            >
              {/* Navigation Tabs - More compact */}
              <div className="flex mb-4 border-b border-gray-800">
                <button
                  onClick={() => setActiveTab(1)}
                  className={`px-3 py-1 text-sm font-medium flex-1 text-center ${
                    activeTab === 1
                      ...
                      <div className="flex flex-col space-y-3">
                        {[
                          ...
                          <Link
                            key={item.path}
                            to={item.path}
                            className="flex items-center py-2 text-lg text-white hover:text-red-400 font-medium"
                            onClick={() => setIsOpen(false)}
                          >
              ...
              <div className="mt-3 pt-3 border-t border-gray-800">
                ...
                <Link
                  to="/get-noticed"
                  className="flex items-center justify-center gap-2 px-4 py-3 rounded-lg text-white font-bold text-lg relative group"
                  onClick={() => setIsOpen(false)}
                >
                  {/* Blur background */}
                  <div className="absolute inset-0 bg-white/10 backdrop-blur-sm rounded-lg group-hover:bg-white/15 transition-all duration-300 -z-10" />
        ...
        100% { transform: rotate(360deg); }
                ...
                {/* Bottom Links - More compact */}
                <div className="flex justify-around mt-1 py-2  text-[0.65rem]">
                  {[
                    { path: "/qr", label: "Share Us", icon: QrCode },
                    ...
                    <Link
                      ...
                      onClick={() => setIsOpen(false)}
                    >
                      <item.icon className="w-4 h-4 mb-1 text-red-500" />
                      <span>{item.label}</span>
                    </Link>
                  ))}
                </div>
