        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {teamMembers.map((member) => (
              <motion.div
                key={member.name}
                className={`team-member p-6 bg-black/20 backdrop-blur-sm border border-gray-800 rounded-xl transition-all duration-300 ${
                  expandedMember === member.name ? 'scale-105' : ''
                }`}
...
>
 ...
 <span
        className={`text-sm transition-colors duration-300 ${
          expandedMember === member.name ? 'text-white' : 'text-red-500'
        }`}
      >
