...
const MAX_ARTICLES_BEFORE_PAGINATION = 9;
...
const ArticlesPage: React.FC = () => {
  ...
  const filteredArticles = useMemo(() => {
    ...
    if (term) {
      return articles.filter((article) => {
        return (
          ...
          (article.subCategory &&
            article.subCategory.toLowerCase().includes(term)) ||
          (article.tags &&
    ...
    // When no search term, respect category filters
    return articles.filter((article) => {
      ...
      const matchesSubCategory =
        !activeSubCategory || article.subCategory === activeSubCategory;
  ...
  const { categories, categorySubMap } = useMemo(() => {
    ...
    const map: Record<Category, SubCategory[]> = {} as Record<
      Category,
      SubCategory[]
    >;
    ...
    articles.forEach((article) => {
      ...
      if (
        article.subCategory &&
        !map[article.category].includes(article.subCategory)
      ) {
  ...
  const handleLoadMore = useCallback(() => {
    setVisibleCount((prev) =>
      Math.min(prev + LOAD_MORE_COUNT, filteredArticles.length)
    );
  }, [filteredArticles.length]);
  ...
  const handleCategoryClick = (category: Category | null) => {
    setSearchTerm(""); // Clear search when clicking a category
  ...
  useEffect(() => {
    ...
    // Just for reference - doesn't affect scroll position
    const element = articlesSectionRef.current;
    // ... any other pagination logic without scroll
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="keywords"
          content="Urban Hustle Films articles, film industry insights, filmmaking blog, indie filmmaking stories, Bishanpreet Singh, UHF content, behind the scenes, film production tips"
        />
        <link rel="canonical" href="https://uhfilms.in/articles" />
        ...
        <div className="fixed inset-0 overflow-hidden pointer-events-none">
          <div
            ...
            style={{
              ...
              height: "80vh",
              background:
                "radial-gradient(circle, rgba(239, 68, 68, 0.4) 0%, rgba(0,0,0,0) 70%)",
          ...
          >
            <div className="max-w-4xl">
              ...
              >
                ...
                of articles covering everything from{" "}
                <span className="text-red-300">behind-the-scenes magic</span> to{" "}
                <span className="text-purple-300">industry trends</span> and{" "}
                <span className="text-blue-300">analysis</span>.
              </motion.p>
  ...
  `}</style>
        ...
        <section
          ...
          className="relative py-12 md:py-16 bg-gradient-to-b from-gray-900/0 via-gray-900/30 to-gray-900"
        >
              ...
              >
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <CategoryFilter
                    ...
                    subCategories={
                      activeCategory ? categorySubMap[activeCategory] : []
                    }
                  ...
                  {usePagination ? (
                    <div className="flex items-center gap-3 px-6 py-3 rounded-full bg-gray-800/80 backdrop-blur-sm border border-gray-700/50 shadow-lg">
                      ...
                      <span className="px-4 text-sm font-semibold text-white tracking-wider">
                        Page {currentPage}{" "}
                        <span className=" text-red-400 ">of</span> {totalPages}
                      </span>
